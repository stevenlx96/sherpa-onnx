...
2. **说出唤醒词** (如 "你好小智") → 检测到后自动切换到 ACTIVE 模式
3. **语音识别** → ASR 实时转写，VAD 智能断句
4. **自动休眠** → 5 秒无语音后自动返回 STANDBY 模式
5. **循环往复** → 等待下一次唤醒
...
A: **是的**。没有 `keywords.txt` 文件，KWS 无法启动，应用会自动降级到直接识别模式（无唤醒功能）。

### Q: 如何知道 KWS 是否启用？
A: 启动应用后，状态栏会显示：
- `✓ KWS已启用 (唤醒词检测)` - KWS 已加载
- `✗ KWS未加载 (直接识别模式)` - KWS 未加载
