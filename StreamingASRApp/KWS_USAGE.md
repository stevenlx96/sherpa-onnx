...
**重要特性：**
- 启动时 **不会同时加载** KWS 和 ASR，避免资源冲突
- KWS 模式下，ASR 在唤醒后 **延迟加载**，休眠后 **自动释放**
- 节省内存和电量，避免 pthread_mutex 冲突导致的崩溃
...
[STANDBY]  仅 KWS 常驻内存 (~50MB)
    ↓ 唤醒
[ACTIVE]   KWS + ASR 同时在内存 (~150MB)
    ↓ 5秒无语音
[STANDBY]  释放 ASR，仅 KWS 常驻 (~50MB) ✓ 节省 100MB
```
...
### Q: 之前版本崩溃了，现在修复了吗？
A: **已修复**！之前的 `pthread_mutex_lock` 崩溃是因为同时加载 KWS 和 ASR 导致资源冲突。
- ✓ 现在启动时只加载一种识别器
- ✓ KWS 模式下 ASR 延迟加载（唤醒后）
- ✓ 自动释放未使用的识别器节省内存
